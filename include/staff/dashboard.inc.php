...
<section class="content-header">
    <h1><?php echo __('Agent Dashboard'); ?></h1>
</section>
            ...
            <div class="col-xs-12">
                <div class="box">
                    <div class="box-header with-border">
                        <h3 class="box-title"><?php echo __('Select the starting time and period for the system activity graph'); ?></h3>
                    </div>
                    <div class="box-body">
                        <div class="form-group">
                            <label for="start" class="col-sm-4 control-label"><?php echo __('Report timeframe'); ?></label>
                            <div class="col-sm-6">
                                <input type="text" class="form-control dp input-medium search-query" name="start" id="start" placeholder="<?php echo __('Last month'); ?>" value="<?php echo Format::htmlchars($report->getStartDate()); ?>" />
                            </div>
                        </div>
                    </div>
...
foreach ($groups as $g => $desc) {
    ...
    <div class="tab_content <?php echo (!$first) ? 'hidden' : ''; ?>" id="<?php echo Format::slugify($g); ?>">
        <table class="dashboard-stats table"><tbody><tr>
                    <?php foreach ($data['columns'] as $j => $c) { ?>
